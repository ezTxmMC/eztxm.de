...
<template>
  ...
  >
    ...
    <div class="mt-auto space-y-3">
      ...
      <RouterLink
        v-if="repository.owner?.login && repository.owner.login !== 'ezTxmMC'"
        :to="`/organization/${repository.owner.login}`"
        @click.stop
        class="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-medium text-slate-300 transition hover:border-violet-400/50 hover:bg-violet-500/10 hover:text-violet-300"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="14"
